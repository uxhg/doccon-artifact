...
def extractREQEQFacts(sentence, contract_fqn, func_name) -> (List[str], Set[RequireFact]):
    ...
    operator = 'EQ'
    if ' must be owned by ' in sentence:
    ...
    if ' must have the same length' in sentence:
...
def extractREQNEQFacts(sentence, contract_fqn, func_name) -> (List[str], Set[RequireFact]):
    ...
    dl_facts: Set[RequireFact] = set()
    operator = 'NEQ'
    if ' cannot be ' in sentence:
        right = sentence.split('cannot be')[-1].strip().strip(',').strip('.')
        if '`' in right:
            right = right.split('`')[1]
        # right = right.replace('`', '')
        if right == 'the zero address':  # template T2
            right = 'address(0)'
        if right == 'zero':  # template T6
            right = '0'
...
def extractREQLEQFacts(sentence, contract_fqn, func_name) -> (List[str], Set[RequireFact]):
    ...
    right = right.replace('`', '')
    if right == 'the fee denominator':  # template T4
...
def extractREQGEQFacts(sentence, contract_fqn, func_name) -> (List[str], Set[RequireFact]):
    ...
    for lv in left_vars:
        lv = lv.replace('`', '')
        if 'have a balance of' in left:
            if '`' + lv + '`' not in left.split('have a balance of')[0]:
                continue
            lv += '_balance'
        elif 'have allowance for' in left:
            lv += '_allowance'
...
def extractREQLTFacts(sentence, contract_fqn, func_name)-> (List[str], Set[RequireFact]):
    ...
    dl_facts: Set[RequireFact] = set()
    operator = 'LT'
    right = sentence.split('must be strictly less than')[-1].strip().strip(',').strip('.')
    right = right.replace('`', '')
...
def extractREQFacts(sentence, contract_fqn, func_name)-> (List[str], Set[RequireFact]):
    sentence_facts = []
    dl_facts: Set[RequireFact] = set()
    if ' must be already minted' in sentence:
    ...
    if ' must not exist' in sentence:
        ...
        for lv in left_vars:
            ...
            if fact not in sentence_facts:
                sentence_facts.append(fact)
    if ' the contract must not be paused' in sentence:
        fact = 'FUNCTION,PLACEHOLDER,REQUIRE,!paused()'
        dl_facts.add(RequireFact(contract_fqn, func_name, NegExpr(Expr(f"paused()"))))
        if fact not in sentence_facts:
            sentence_facts.append(fact)
    '''
    if ' must implement ' in sentence:
        right = sentence.split('must implement ')[-1].strip().strip(',').strip('.')
        words = [word.strip().strip(',').strip('.') for word in right.split()]
        right_vars = [word[1:-1].replace('-', '.') for word in words if word[0] == '{' and word[-1] == '}']
        left = sentence.split('- ')[-1].split(' must implement ')[0].strip().strip(',').strip('.')
        left_vars = [word for word in left.split() if word[0] == word[-1] == '`']
        for lv in left_vars:
            lv = lv.replace('`', '')
            for rv in right_vars:
                lv = '_check' + rv.split('.')[-1] + '(' + lv + ')'
                fact = 'FUNCTION,PLACEHOLDER,REQUIRE,' + lv
            dl_facts.add(RequireFact(contract_fqn, func_name, Expr(lv)))
            if fact not in sentence_facts:
                sentence_facts.append(fact)
