...
from datalog.pred import SeeFnFact, HasFnFact, RequireFact, HasParamFact, \
    RevertFact, EmitFact, FnModFact, OverrideFact
...
def extractDocFactsFromOneSrcFile_OpenZeppelin(lib, version, src_file):
    ...
    #    with open(output_json_file, 'r') as fr:
    ...
    # contract-level
    doc_facts, dl_has_fn = extractContractContainFacts_OpenZeppelin(lib, version, src_file,
                                                                    doc_facts)
    ...
    for i in range(len(lines)):
        ...
        if lines[i].strip() == '*/':
            ...
            if lines[j].strip().startswith('function '):
                ...
                # print(func_name, ''.join(doc_lines))
                # func-level
                doc_facts, see_fn_facts, require_facts, emit_facts, \
                revert_facts, has_param_facts = extractDocFactsFromOneFunc_OpenZeppelin(
    ...
    write_dl_facts(dl_facts_dir, dl_has_fn, dl_see_fn, dl_require, dl_emit,
                   dl_revert, dl_has_param, set(), set())
...
def extractDocFactsFromOneSrcFile_ERC721(lib, version, src_file):
    ...
    for i in range(len(lines)):
        ...
        if lines[i].strip().startswith('///') and not lines[i + 1].strip().startswith('///'):
            ...
            if lines[j].strip().startswith('function '):
                ...
                # print(func_name, ''.join(doc_lines))
                # func-level
                doc_facts, fn_mod_facts, require_facts, emit_facts, revert_facts, has_param_facts, override_facts = \
                    extractDocFactsFromOneFunc_ERC721(lib, version, doc_lines, contract_fqn, func_name, doc_facts)
                ...
                pass
                # func_name = lines[j].strip().split('modifier ')[-1].split('(')[0]
                # doc_lines = lines[start_idx: end_idx + 1]
                # doc_facts = extractDocFactsFromOneFunc_ERC721(lib, version, doc_lines, contract_fqn, func_name, doc_facts)
            elif lines[j].strip().startswith('constructor '):
                pass
                # func_name = lines[j].strip().split('constructor ')[-1].split('(')[0]
                # doc_lines = lines[start_idx: end_idx + 1]
                # doc_facts = extractDocFactsFromOneFunc_ERC721(lib, version, doc_lines, contract_fqn, func_name, doc_facts)
            else:
                # we do not analyze struct doc for now
    ...
    write_dl_facts(dl_facts_dir, dl_has_fn, set(), dl_require, dl_emit,
                   dl_revert, dl_has_param, dl_fn_mod, dl_override)
    with open(output_json_file, 'w') as fw:
        json.dump(doc_facts, fw, indent=2)
...
def extractDocFactsFromOneReadmeFile_DappHub(lib, version, src_file):
    ...
    for i in range(len(lines)):
        if lines[i].strip().startswith('<h2>'):
            ...
            doc_facts = []
            # contract-level
            # doc_facts = extractContractContainFacts_DappHub(lib, version, src_file, doc_facts)
        if lines[i].strip().startswith('#### `'):
            start_idx = i + 1
            # print(start_idx)
        if start_idx != -1 and lines[i].strip() == '':
            ...
            # print(func_name, ''.join(doc_lines))
            doc_facts, require_facts, emit_facts, override_facts, fn_mod_facts = extractDocFactsFromOneFunc_DappHub(lib, version, doc_lines, contract_fqn, func_name, doc_facts)
            dl_require = dl_require.union(require_facts)
            dl_emit = dl_emit.union(emit_facts)
            dl_override = dl_override.union(override_facts)
            dl_fn_mod = dl_fn_mod.union(fn_mod_facts)
    ...
    write_dl_facts(dl_facts_dir, dl_has_fn, set(), dl_require, dl_emit, set(), set(), dl_fn_mod, dl_override)
    # for fact in doc_facts:
    #    print(fact)
